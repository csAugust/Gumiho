...
class KVCache:
    ...
    def filter(self, keep_indices: torch.Tensor, dim: int = 2):
        """
        Filter the KV cache to keep only specified positions and discard the rest.
        This is an efficient operation that reorganizes the cache in-place by copying
        only the desired positions and updating the current_length.

        Args:
            keep_indices (torch.Tensor): 1D tensor of indices to keep in the cache.
            dim (int, optional): Dimension along which to filter. Default is 2 (sequence dimension).

        Returns:
            int: The new length of the cache after filtering.
        """
        ...
        # Validate indices are within current bounds
        if keep_indices.max() >= self.current_length.item():
            raise ValueError(f"keep_indices contains values >= current_length ({self.current_length.item()})")

        # Use the existing copy method to reorganize the cache
        # This is efficient as it only copies the desired positions
        self.copy(keep_indices, prev_length=0, dim=dim)

        return self.current_length.item()
    
    
def rollback_past_key_values(past_key_values, new_total_length):
    for layer_idx in range(len(past_key_values)):
        for kv_idx in range(2):  # 0 for key, 1 for value
            past_key_values[layer_idx][kv_idx].current_length.fill_(new_total_length)
